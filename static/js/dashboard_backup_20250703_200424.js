...
function updateMetrics(data) {
    ...
    // Проверяем наличие данных от умного калькулятора - ТОЛЬКО если они валидны и содержат корректные значения
    if (data.real_activity_stats && 
        data.real_activity_stats.total_time > 0 && 
        typeof data.real_activity_stats.total_time === 'number' &&
        typeof data.real_activity_stats.active_time === 'number' &&
        typeof data.real_activity_stats.passive_time === 'number') {
        ...
        const productiveMinutes = Math.round(stats.productive_time || 0); 
        const activityPercent = Math.round(stats.activity_ratio * 100);
        const productivityPercent = Math.round(stats.productivity_score || 0); 
    ...
    updateMetricsInInterface({
        totalTime: metrics.totalTime > 0 ? metrics.totalTime : (lastValidMetrics ? lastValidMetrics.totalTime : 0),
        productiveTime: metrics.productiveTime > 0 ? metrics.productiveTime : (lastValidMetrics ? lastValidMetrics.productiveTime : 0),
        productivityScore: metrics.productivityScore > 0 ? metrics.productivityScore : (lastValidMetrics ? lastValidMetrics.productivity : 0),
        breaks: metrics.breaks || (lastValidMetrics ? lastValidMetrics.breaks : 0),
    ...
    // Проверка на наличие данных перед обновлением метрик
        ...
        try { 
            ...
            if (typeof updateMainMetricsWithRealActivity === 'function') {
                ...
                // ФИНАЛЬНАЯ ЛОГИКА: Передаем ВСЕ данные, умный калькулятор сам правильно их обработает
                console.log('🔄 Передаем в умный калькулятор ВСЕ данные для правильного слияния:');
                ...
                // Умный калькулятор получает windowData и сам берет browserData из window.lastLoadedData
